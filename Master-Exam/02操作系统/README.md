...
4. **管程**是由一组数据及定义在这组数据之上的对这组数据的操作组成的软件模块，这组操作能初始化并改变管程中的数据和进程同步。管程不仅能实现进程间的互斥，而且能实现进程间的同步。一个进程只有通过调用管程内的过程才能进入管程访问共享数据。
4. **Peterson算法：**为了防止两个进程为进入临界区而无限期等待，设置了变量`turn`表示允许进入临界区的编号。每个进程在先设置自己的标志后再设置`turn`标志，允许另一个进程进入。这时，再同时检测另一个进程状态标志和允许进入标志，就可以保证两个进程同时要求进入临界区时只允许一个进程进入。保存的事较晚一次复制，因此较晚的进程等待，较早的进程进入。先到先入，后到等待。
4. 信箱通信是一种间接通信方式。
